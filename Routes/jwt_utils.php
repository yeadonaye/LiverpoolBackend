<?php
	...
	function get_jwt_payload($jwt) {
		$tokenParts = explode('.', $jwt);
		if(count($tokenParts) !== 3) return null; // JWT doit comporter 3 parties (header, payload, secret)
		return json_decode(base64_decode($tokenParts[1]), true); // Retourne le payload
	}
	
	// Methodes pour verifier si l'utilisateur est un coach ou un joueur en verifiant le token
	function is_coach($jwt, $secret) {
		if(!is_jwt_valid($jwt, $secret)) return false; // Si le token n'est pas valide, on considère que l'utilisateur n'est pas un coach
		$payload = get_jwt_payload($jwt);
		return isset($payload['role']) && $payload['role'] === 'coach';
	}

	function is_joueur($jwt, $secret) {
		if(!is_jwt_valid($jwt, $secret)) return false; // Si le token n'est pas valide, on considère que l'utilisateur n'est pas un joueur
		$payload = get_jwt_payload($jwt);
